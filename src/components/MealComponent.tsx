    ...
    <div className="p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-6 pb-4 border-b border-slate-700">
        <div className="flex-grow min-w-0">
            {isEditingName ? (
                ...
                <button onClick={() => setIsEditingName(true)} className="ml-2 p-1.5 text-slate-400 hover:text-teal-400" aria-label="Edit meal name"><Edit3 size={20} /></button>
                </div>
            )}
      ...
      <div className="p-4 bg-slate-700/50 rounded-lg border border-slate-600/60 mb-6">
        ...
        <div className="flex flex-col sm:flex-row gap-x-6 gap-y-2 mb-3 text-sm">
            <label className="flex items-center text-slate-300 cursor-pointer py-1">
                ...
                <Database size={18} className="mr-1.5 text-green-400"/> Generic Foods (USDA)
            </label>
            {!searchGenericOnly && (
                <label className="flex items-center text-slate-300 cursor-pointer py-1">
                    ...
                    <ShoppingBasket size={18} className="mr-1.5 text-sky-400" /> Packaged Foods Found in Stores
                </label>
            )}
        </div>
              ...
              return (
                <div key={itemId} className="p-2.5 bg-slate-600 hover:bg-slate-500/80 rounded-md flex justify-between items-center text-sm gap-2">
                  <div className="flex-grow min-w-0 flex items-center">
                    {item.source === 'USDA' ? <Database size={18} className="text-green-400 mr-2 flex-shrink-0"/> : <ShoppingBasket size={18} className="text-sky-400 mr-2 flex-shrink-0"/>}
                    <div>
                  ...
                  <button onClick={() => handleAddFoodFromSearch(item)} className="bg-green-500 hover:bg-green-600 text-white text-xs font-medium py-1.5 px-3 rounded-md shadow transition-colors flex-shrink-0"><PlusCircle size={18} className="inline mr-1"/> Add</button>
                </div>
              );
            })}
          </div>
      ...
      {/* Ingredients Table */}
      <div className="overflow-x-auto rounded-lg border border-slate-700 shadow-inner bg-slate-800/60">
        <table className="min-w-full divide-y divide-slate-700">
          <thead className="bg-slate-700/50">
            <tr>
              <th scope="col" className="w-1/3 px-4 py-3 text-left text-sm font-semibold text-slate-300 uppercase tracking-wider">Ingredient</th>
              <th scope="col" className="px-3 py-3 text-center text-sm font-semibold text-slate-300 uppercase tracking-wider">Cal/100g</th>
              <th scope="col" className="px-3 py-3 text-center text-sm font-semibold text-slate-300 uppercase tracking-wider">Prot/100g</th>
              <th scope="col" className="px-3 py-3 text-center text-sm font-semibold text-slate-300 uppercase tracking-wider">Fat/100g</th>
              <th scope="col" className="px-3 py-3 text-center text-sm font-semibold text-slate-300 uppercase tracking-wider">Carb/100g</th>
              <th scope="col" className="px-3 py-3 text-center text-sm font-semibold text-slate-300 uppercase tracking-wider">Grams</th>
              <th scope="col" className="px-3 py-3 text-center text-sm font-semibold text-slate-300 uppercase tracking-wider">Calories</th>
              <th scope="col" className="px-3 py-3 text-center text-sm font-semibold text-slate-300 uppercase tracking-wider">Protein</th>
              <th scope="col" className="px-3 py-3 text-center text-sm font-semibold text-slate-300 uppercase tracking-wider">Fat</th>
              <th scope="col" className="px-3 py-3 text-center text-sm font-semibold text-slate-300 uppercase tracking-wider">Carbs</th>
              <th scope="col" className="px-4 py-3 text-center text-sm font-semibold text-slate-300 uppercase tracking-wider">Action</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-700/60">
            {currentMeal.ingredients.length === 0 && (
              <tr>
                <td colSpan={11} className="px-6 py-8 text-center text-slate-400 italic">
                  No ingredients added yet. Add some using search or manually.
                </td>
              </tr>
            )}
            {currentMeal.ingredients.map((ing, index) => (
              <IngredientRowComponent
                key={ing.id}
                ingredient={ing}
                onUpdate={(updatedValues) => handleUpdateIngredient(ing.id, updatedValues)}
          ...
          {currentMeal.ingredients.length > 0 && (
            <tfoot className="bg-slate-700/50">
              <tr>
                <td className="px-4 py-3 text-left font-semibold text-slate-100 text-sm">Total</td>
                <td colSpan={5}></td> {/* Empty cells for 100g inputs */}
                <td className="px-3 py-3 text-center font-semibold text-slate-100 text-sm">{round(mealTotals.calories)}</td>
                <td className="px-3 py-3 text-center font-semibold text-slate-100 text-sm">{round(mealTotals.protein)}</td>
                <td className="px-3 py-3 text-center font-semibold text-slate-100 text-sm">{round(mealTotals.fat)}</td>
                <td className="px-3 py-3 text-center font-semibold text-slate-100 text-sm">{round(mealTotals.carbs)}</td>
      ...
      <button onClick={handleManualAddIngredient} className="mt-6 bg-green-600 hover:bg-green-700 text-white font-medium py-2.5 px-5 rounded-lg shadow flex items-center justify-center w-full sm:w-auto"><PlusCircle size={20} className="mr-2"/> Add Ingredient Manually</button>
      
      <div className="mt-8 pt-6 border-t border-slate-700 flex flex-col sm:flex-row justify-end gap-3">
        {editingMode === 'instance' && onSaveAsTemplate && (
           <button onClick={() => onSaveAsTemplate(currentMeal as MealInstance)} className="bg-purple-600 hover:bg-purple-700 text-white font-medium py-2.5 px-5 rounded-lg shadow-md flex items-center justify-center"><Star size={18} className="mr-2"/> Save as New Template</button>
        )}
        <button onClick={handleFinalSave} className="bg-teal-600 hover:bg-teal-700 text-white font-medium py-2.5 px-5 rounded-lg shadow-md flex items-center justify-center"><Save size={18} className="mr-2"/> Save {editingMode === 'template' ? 'Template' : 'Meal to Day'}</button>
        <button onClick={onClose} className="bg-slate-600 hover:bg-slate-500 text-slate-100 font-medium py-2.5 px-5 rounded-lg shadow-md">Close</button>
      </div>
    </div>
  );
};
