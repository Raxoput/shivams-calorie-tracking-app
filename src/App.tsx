  ...
  const loadUserData = async (currentUser: User) => {
    ...
    try {
      ...
      if (mealLibrarySnap.exists()) {
        currentTemplates = (mealLibrarySnap.data().meals || []).map((m: MealTemplate) => ({ ...m, createdAt: m.createdAt || Date.now() })).sort((a: MealTemplate, b: MealTemplate) => (a.createdAt || 0) - (b.createdAt || 0));
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-950 text-slate-100 p-2 md:p-4 lg:p-6 selection:bg-teal-500 selection:text-white">
          ...
          {user ? (
            <>
              ...
              <button onClick={logOut} className="bg-red-600 hover:bg-red-700 text-white font-medium py-2 px-4 rounded-lg text-sm shadow-md hover:shadow-lg transition-all duration-150 flex items-center group">
                <LogOutIcon size={20} className="mr-2 group-hover:animate-pulse-gentle-hover" /> Log Out
              </button>
            </>
          ) : (
        ...
        <main className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6 px-1">
            <div className="flex items-center gap-2">
              <button onClick={goToPreviousWeek} className="p-3 bg-slate-700 hover:bg-slate-600 rounded-lg text-slate-200 shadow transition-colors" aria-label="Previous week"><ChevronLeft size={24} /></button>
              ...
              <button onClick={goToNextWeek} className="p-3 bg-slate-700 hover:bg-slate-600 rounded-lg text-slate-200 shadow transition-colors" aria-label="Next week"><ChevronRight size={24} /></button>
            </div>
            <div className="flex flex-col sm:flex-row gap-3">
              <button onClick={() => {setIsMealLibraryModalOpen(true); setExpandedDayKey(null);}} className="bg-purple-600 hover:bg-purple-700 text-white font-medium py-2.5 px-5 rounded-lg shadow flex items-center justify-center"><BookOpen size={20} className="mr-2"/> Meal Library</button>
              <button onClick={() => setIsShoppingListModalOpen(true)} className="bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2.5 px-5 rounded-lg shadow flex items-center justify-center"><ShoppingCart size={20} className="mr-2"/> Shopping List</button>
            </div>
          </div>
                ...
                >
                        ...
                        <div key={assignment.instanceId} className={`bg-slate-700/70 p-2 rounded text-xs flex justify-between items-center group hover:bg-slate-600/90 ${isExpanded ? 'py-2.5 px-3' : 'py-1.5 px-2'}`}>
                          ...
                          {isExpanded && (
                            <div className="flex items-center gap-1">
                                <button onClick={(e) => {e.stopPropagation(); handleEditMealInstance(assignment.instanceId, dateKey);}} className="p-1.5 text-slate-300 hover:text-teal-400" aria-label={`Edit ${instanceName}`}><Edit size={18}/></button>
                                <button onClick={(e) => {e.stopPropagation(); handleRemoveMealFromDay(assignment.instanceId, dateKey);}} className="p-1.5 text-slate-400 hover:text-red-400" aria-label={`Remove ${instanceName}`}><Trash2 size={18}/></button>
                            </div>
                          )}
                        </div>
                      );
                  ...
                  {isExpanded && (
                    <div className="mt-auto pt-3 border-t border-slate-700 flex flex-col sm:flex-row gap-2">
                        <button onClick={(e) => {e.stopPropagation(); handleOpenQuickAddMeal(dateKey);}} className="bg-sky-600 hover:bg-sky-700 text-white text-sm font-medium py-2 px-3 rounded-md w-full flex items-center justify-center"><PlusCircle size={18} className="mr-1.5"/> Add Quick Meal</button>
                        <button onClick={(e) => {e.stopPropagation(); setAssignTemplateToDate(dateKey); setExpandedDayKey(null);}} className="bg-purple-600 hover:bg-purple-700 text-white text-sm font-medium py-2 px-3 rounded-md w-full flex items-center justify-center"><BookOpen size={18} className="mr-1.5"/> Add from Library</button>
                    </div>
                  )}
                   {!isExpanded && (
                     <button onClick={(e) => {e.stopPropagation(); setExpandedDayKey(dateKey);}} className="mt-auto bg-teal-600/80 hover:bg-teal-600 text-white text-xs font-medium py-1.5 px-2 rounded-md w-full transition-colors opacity-80 hover:opacity-100">
                         <Eye size={18} className="inline mr-1"/> View/Edit Day
                     </button>
                   )}
                </div>
              );
      ...
      {isMealLibraryModalOpen && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-40 flex items-center justify-center p-4 animate-fadeIn" onClick={() => setIsMealLibraryModalOpen(false)}>
          <div className="bg-slate-800 p-5 sm:p-6 rounded-xl shadow-2xl w-full max-w-2xl border border-slate-700 max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <button onClick={handleOpenCreateMealTemplate} className="mb-4 bg-green-600 hover:bg-green-700 text-white font-medium py-2.5 px-4 rounded-lg shadow flex items-center justify-center w-full sm:w-auto self-start"><PlusCircle size={20} className="mr-2"/> Create New Meal Template</button>
              ...
              {allMealTemplates.map(template => (
                <div key={template.id} className="bg-slate-700/80 p-3 rounded-lg flex justify-between items-center group">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => {setEditingTemplate(template); setIsMealLibraryModalOpen(false);}} className="p-2 text-slate-300 hover:text-teal-400" aria-label={`Edit ${template.name}`}><Edit size={20}/></button>
                    <button onClick={() => handleRemoveMealTemplate(template.id)} className="p-2 text-slate-300 hover:text-red-400" aria-label={`Delete ${template.name}`}><Trash2 size={20}/></button>
                  </div>
                </div>
              ))}
            </div>
            ...
            {shoppingListItems.length === 0 ? (
              ...
              <>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center gap-3 pt-4 border-t border-slate-700">
                  <button onClick={clearCheckedShoppingListItems} disabled={!shoppingListItems.some(i => i.checked)} className="bg-red-600 hover:bg-red-700 text-white font-medium py-2.5 px-5 rounded-lg shadow w-full sm:w-auto disabled:opacity-50"><Trash2 size={20} className="mr-2 inline"/> Clear Checked</button>
                  <button onClick={printShoppingList} disabled={shoppingListItems.length === 0} className="bg-sky-600 hover:bg-sky-700 text-white font-medium py-2.5 px-5 rounded-lg shadow w-full sm:w-auto disabled:opacity-50"><Printer size={20} className="mr-2 inline"/> Print List</button>
                </div>
              </>
            )}
          </div>
      ...
      <footer className="text-center text-slate-500 mt-16 py-6 border-t border-slate-700/50 text-sm">
          <p>&copy; {new Date().getFullYear()} Shivam's Calorie Tracking App.
